...
/*   Updated: 2024/04/10 13:27:10 by yitoh         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_checkmap(char **tmp, int i, int k, int c_cnt)
{
	...
	{
		k = 0;
		while (tmp[i] && tmp[i][k] && ft_strchr(" \t", tmp[i][k]))
			k++;
		if ((!ft_strncmp(tmp[i] + k, "NO", 2)
				|| !ft_strncmp(tmp[i] + k, "SO", 2)
				|| !ft_strncmp(tmp[i] + k, "WE", 2)
				|| !ft_strncmp(tmp[i] + k, "EA", 2)) && tex(tmp[i], k + 2))
			return (1);
		else if (tmp[i] && (tmp[i][k] == 'F' || tmp[i][k] == 'C'))
		{
			c_cnt++;
			if (c_cnt > 2 || (check_color(tmp[i], k + 1)))
				return (1);
		}
		else if (tmp[i] && !ft_strchr("NSWEFC\n\0", tmp[i][k]))
		{
